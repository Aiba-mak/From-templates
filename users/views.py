...
from .serializers import UserRegisterSerializer, UserUpdateSerializer, ProfileUpdateSerializer
import random
...
def register(request):
	if request.method == 'POST':
		serializer = UserRegisterSerializer(request.POST)
...
def edit_profile(request):
	if request.method == 'POST':
		u_form = UserUpdateSerializer(request.POST, instance=request.user)
		p_form = ProfileUpdateSerializer(request.POST, request.FILES, instance=request.user.profile)
		...
		u_form = UserUpdateSerializer(instance=request.user)
		p_form = ProfileUpdateSerializer(instance=request.user.profile)
	context ={
		'u_form': u_form,
		'p_form': p_form,
	}
